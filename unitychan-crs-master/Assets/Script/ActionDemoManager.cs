...
//		iTween.MoveTo (this.hand, this.position_list[order[0]], 0);
	...
	private void SimpleAction(int current, int before, float time, int action_count){
		...
		if (current == (int)ActionManager.Icon.POINTER_UP) {
			//タップ演出
		} else if (before == (int)ActionManager.Icon.TOP_LEFT && current == (int)ActionManager.Icon.TOP_RIGHT) {
			Debug.Log(1);
...
//				"delay", time * action_count));
			iTween.MoveTo (this.hand, iTween.Hash (
				...
				"time", time,
				"delay", time * action_count));
		} else if (before == (int)ActionManager.Icon.TOP_RIGHT && current == (int)ActionManager.Icon.TOP_LEFT) {
			Debug.Log(2);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
		} else if (before == (int)ActionManager.Icon.BOTTOM_LEFT && current == (int)ActionManager.Icon.BOTTOM_RIGHT) {
			Debug.Log(3);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
		} else if (before == (int)ActionManager.Icon.BOTTOM_RIGHT && current == (int)ActionManager.Icon.BOTTOM_LEFT) {
			Debug.Log(4);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
