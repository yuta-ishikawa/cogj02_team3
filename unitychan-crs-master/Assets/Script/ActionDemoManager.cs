using UnityEngine;
using System.Collections;
using System.Collections.Generic;

public class ActionDemoManager : MonoBehaviour {

	private StageManager stageManager;
	private GameObject hand;
	private List<Vector3> position_list = new List<Vector3>();
	private Vector3 top_left;
	private Vector3 top_center;
	private Vector3 top_right;
	private Vector3 bottom_left;
	private Vector3 bottom_center;
	private Vector3 bottom_right;
	private Vector3 middle_center;

	public enum Icon{
		POINTER_UP = -1,
		TOP_LEFT,
		TOP_CENTER,
		TOP_RIGHT,
		BOTTOM_LEFT,
		BOTTOM_CENTER,
		BOTTOM_RIGHT,
		MIDDLE_CENTER,
	}

	// Use this for initialization
	void Start () {
		this.hand = GameObject.Find ("hand");

		this.position_list.Add(GameObject.Find("top_left").transform.position);
		this.position_list.Add(GameObject.Find("top_center").transform.position);
		this.position_list.Add(GameObject.Find("top_right").transform.position);
		this.position_list.Add(GameObject.Find("bottom_left").transform.position);
		this.position_list.Add(GameObject.Find("bottom_center").transform.position);
		this.position_list.Add(GameObject.Find("bottom_right").transform.position);
		this.position_list.Add(new Vector3(222, 125, 0));

		
		// List<int> order = new List<int>{ 0, 3, 1, 4, 2, 5 };
		// List<int> order = new List<int>{ 0, 2, 0, 3, 5, 5, 3 };
		// // List<int> order = new List<int>{ 0, 2 };
		// float time = 10.0f;
		// Next (order, "", time);
	}
	
	// Update is called once per frame
	void Update () {
	
	}

	public void Next(List<int> order, string motionName, float time) {
		switch (motionName) {
		case "dummy":
			break;
		default:
			SimpleActionDemo(order, time/order.Count);  // orderに基づいた簡易アニメーション 
			break;
		}
	}


	private void SimpleActionDemo(List<int> order, float one_action_time) {
//		iTween.MoveTo (this.hand, this.position_list[order[0]], 0);
		this.hand.transform.position = this.position_list[order[0]];
		for(int i =0; i<order.Count; i++){
			if (i == 0) {
				SimpleAction (order [i], order [i], one_action_time, i);
			} else if (order.Count > i) {
				SimpleAction (order [i], order [i - 1], one_action_time, i);
			} else
				LastSimpleAction(order [i], one_action_time, i);
		}
	}


	private void SimpleAction(int current, int before, float time, int action_count){
		Vector3[] movepath = new Vector3[2];
		movepath [0] = this.position_list[(int)Icon.MIDDLE_CENTER];
		movepath [1] = this.position_list[current];

		if (current == (int)Icon.POINTER_UP) {
			//タップ演出
		} else if (before == (int)Icon.TOP_LEFT && current == (int)Icon.TOP_RIGHT) {
			Debug.Log(1);
//			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [(int)Icon.MIDDLE_CENTER],
//				"path", movepath,
//				"time", time / 2,
//				"delay", time * action_count));
			iTween.MoveTo (this.hand, iTween.Hash (
				"path", movepath,
				"time", time,
				"delay", time * action_count));
		} else if (before == (int)Icon.TOP_RIGHT && current == (int)Icon.TOP_LEFT) {
			Debug.Log(2);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
		} else if (before == (int)Icon.BOTTOM_LEFT && current == (int)Icon.BOTTOM_RIGHT) {
			Debug.Log(3);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
		} else if (before == (int)Icon.BOTTOM_RIGHT && current == (int)Icon.BOTTOM_LEFT) {
			Debug.Log(4);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
		} else {
			Debug.Log(5);
			iTween.MoveTo (this.hand, iTween.Hash ("position", this.position_list [current],
				                                   "time", time,
				                                   "delay", time * action_count));
		}
	}


	private void LastSimpleAction(int current, float time, int action_count){
		iTween.MoveTo(this.hand, iTween.Hash("position", this.position_list[current],
			                                 "time", time,
			                                 "delay", time * action_count,
			                                 "oncomplete", "FinishDemo",
			                                 "oncompletetarget", gameObject));
	}


	private void FinishDemo(){
		this.hand.transform.position = new Vector3(0, 300, 0);
		Debug.Log ("FinishDemo");
		stageManager.FinishDemo();
	}
}
